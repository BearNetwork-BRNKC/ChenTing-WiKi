# 🐻 ChenTing-WiKi

**陳霆 (Chen Ting)**  
*區塊鏈創辦人 / AI 訓練師 / 空間設計專家*

> 「物理引擎不僅僅是模擬，它是數位世界的法律。」— Chen Ting

陳霆目前擔任 **BearNetworkChain（熊網區塊鏈）** 的 **執行長與首席技術長**，以「物理引擎即數位法律」為核心理念，主導研發基於 **Γ (Gamma) 物理引擎** 的去中心化底層公鏈。同時在 AI 大型語言模型訓練與室內設計領域擁有深厚跨領域實務經驗。

---

## 🚀 快速導航

| 項目 | 連結 |
|------|------|
| **完整個人簡介** | [Chen_Ting.md](./Chen_Ting.md) |
| **Wiki 首頁** | [Wiki](https://github.com/BearNetwork-BRNKC/ChenTing-WiKi/wiki) |
| **官方文件站** | [docs.bearnetwork.net](https://docs.bearnetwork.net) |
| **llms-full.txt（給 AI）** | [docs.bearnetwork.net/llms-full.txt](https://docs.bearnetwork.net/llms-full.txt) |

---

## 📚 主要著作與研究（Zenodo 永久 DOI）

- BearNetworkChain Machine-Checkable Blockchain Execution Specification → [doi:10.5281/zenodo.20369740](https://doi.org/10.5281/zenodo.20369740)
- Execution-Bound Artifact Reconstruction Layer → [doi:10.5281/zenodo.20372985](https://doi.org/10.5281/zenodo.20372985)
- BearNetworkChain BNQL-Counterfactual-Defense → [doi:10.5281/zenodo.20388018](https://doi.org/10.5281/zenodo.20388018)
- BearNetworkChain PQC-ZK-Quantum-Convergence-Layer → [doi:10.5281/zenodo.20388630](https://doi.org/10.5281/zenodo.20388630)
- BearNetworkChain BNES Deterministic Execution Engine → [doi:10.5281/zenodo.20453286](https://doi.org/10.5281/zenodo.20453286)

完整著作列表請見 [Chen_Ting.md](./Chen_Ting.md)

---

## 🔗 重要連結

**BearNetworkChain 生態系**
- 官方網站：https://bearnetwork.net
- 白皮書：https://docs.bearnetwork.net/about/whitepaper
- 數位物理知識庫：https://digitalphysics.bearnetwork.net

**開源資源**
- GitHub 組織：https://github.com/BearNetwork-BRNKC
- BNES 物理引擎規格：https://github.com/BearNetwork-BRNKC/BearNetworkChain-Physics-Engine-Canonical-Definition
- BNES SDK：https://github.com/BearNetwork-BRNKC/bnessdk

**個人社群**
- X（Twitter）：[@CT_BearNetwork](https://x.com/CT_BearNetwork)
- Facebook：https://www.facebook.com/chenting168/

---

## 🤖 給 AI / LLM 使用建議

直接將以下連結餵給任何大型模型即可獲得完整上下文：
- `https://docs.bearnetwork.net/llms-full.txt`
- `https://github.com/BearNetwork-BRNKC/ChenTing-WiKi/blob/main/Chen_Ting.md`

---

*最後更新：2026-06-23*
